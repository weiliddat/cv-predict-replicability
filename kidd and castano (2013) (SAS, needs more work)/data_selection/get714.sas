...
PROC IMPORT OUT=ToM1
       DATAFILE= "x\KiddCastano\Data&Analysis\data_used\all.xlsx"
       DBMS= xlsx REPLACE;
...
s_pa = pa;
s_age = age;
************RUN THESE EXXLUSIONS FIRST THEN UPDATE TRESHHOLDS USING MEAN AND SD***************************
...
* Time mean=147.8351211 SD=336.1874923 th=147.8351211+(3.5*336.1874923)=1324.49134415;
*if time > 1324.49134415then time_check = 1;
if time_check = 1 then delete;

art_check = 0;
*wrong mean=1.0986301 SD=2.3671560 th=1.0986301+(3.5*2.3671560)=9.3836761 ;
*if wrong > 9.3836761 then art_check = 1;
if art_check = 1 then delete;

score_check = 0;
*update scoretresshold mean=25.6109589 SD=5.4754111 th==25.6109589-(3.5*5.4754111)=6.44702005;
*if score < 6.44702005  then score_check = 1;
