...
PROC IMPORT OUT=ToM1
       DATAFILE= "x\KiddCastano\Data&Analysis\data_used\571.xlsx"
       DBMS= xlsx REPLACE;
...
s_pa = pa;
s_age = age;
************RUN THIS EXXLUSIONS FIRST THEN UPDATE TRESHHOLDS USING MEAN AND SD***************************
**delete all subjects that are excluded from distribution due to skip, previous participation or reading time****
**************DELETE ALL PARTICIPANTS WHO SKIPPED AN RMET QUESTION;
...
*delete everyone who doesnt answer "no" on previous participation;
*if Previous=0 then delete;
*if Previous=1 then delete;
*if Previous=3 then delete;
*MEANS UPDATED

*to avoid drops in sqrt(art) delete all that have an art with less than 0; 
if art < 0 then delete;

time_check = 0;
*delete everyone with too little reading time;
if time =0 then time_check = 1;

*update these means and standard deviations, 3.5*sd+-mean after exclusion has been made; 
* Time mean=160.8109020 SD=406.7720251 th=160.8109020 +(3.5*406.7720251)=1584.51298985;
*if time >1584.51298985 then time_check = 1;
if time_check = 1 then delete;

art_check = 0;
*wrong mean=1.0631808 SD=2.2405394 th=1.0631808 +(3.5*2.2405394)=8.9050687;
*if wrong > 8.9050687 then art_check = 1;
if art_check = 1 then delete;

score_check = 0;
*update scoretresshold mean=25.4509804 SD=5.5692636 th=25.4509804-(3.5*5.5692636)=5.9585578;
*if score < 5.9585578  then score_check = 1;
