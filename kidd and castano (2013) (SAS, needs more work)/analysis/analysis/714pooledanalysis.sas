...
PROC IMPORT OUT=ToM1
       DATAFILE= "x\KiddCastano\Data&Analysis\data_used\714.xlsx"
       DBMS= xlsx REPLACE;
...
*if time < 30 then time_check = 1;

*update these means and standard deviations, 3.5*sd+-mean after exclusion has been made; 
* Time mean=169.4758249 SD=510.7591732 th=169.4758249+(3.5*510.7591732)=1957.1329311;
*if time > 1957.1329311 then time_check = 1;
*if time_check = 1 then delete;

*art_check = 0;
*wrong mean=1.0975610 SD=2.2082708 th=1.0975610+(3.5*2.2082708)=8.8265088;
*if wrong > 8.8265088 then art_check = 1;
*if art_check = 1 then delete;

*score_check = 0;
*update scoretresshold mean=26.0991870 SD=5.0697014 th=26.0991870 -(3.5*5.0697014)=8.3552321;
*if score < 8.3552321 then score_check = 1;
